...
- **S0.8**：分歧率不是一个单一标量，必须拆成三项分别报告，理由是它们诊断的是
  ...
  1. **cluster assignment divergence（含 Ward 事件）**：用
     ...
     （pairwise co-assignment agreement，类似 Rand index 的构造）：对所有
     token 对 `(i,j)`，检查"i、j 在批量路径下是否同簇"与"i、j 在严格串行
     参考下是否同簇"这两个布尔值是否一致，一致的比例就是这一项的分数。
     ...
     **具体做法**：
     - 用**触发这次合并的 orphan 的绝对 token 位置**（两条路径处理的是同
       一份输入，这个位置是两条路径共有、不受路由近似影响的锚点）作为每个
       Ward 事件的主键，按位置对齐两条路径各自的事件列表；
     - 每个事件的"候选对"不用槽号表示，改用**合并前 `keep`/`free` 两个簇
       各自包含的原始 token 绝对位置集合**（`scan_op_log` 扫到这一步即可
       得到）表示——两条路径若在语义上做了同一次合并，这两个集合应该
       （近似）相同，不依赖槽号或 `epoch` 怎么编号；
     - 触发 token 位置在两条路径都出现的事件算作**匹配**，对匹配上的事件
       报告 `keep`/`free` 两个 token 集合各自的 Jaccard 相似度（均值）；
     - 触发 token 位置只在其中一条路径出现的事件，**不要**强行配对或直接
       丢弃平均掉——分别计为 `ward_event_inserted`（只在批量路径）/
  ...
  3. **最终 cache/readout 差异，拆成两个子项，硬性决策门只挂在 3b**——第 1
     ...
     - **3a（诊断，不是决策依据）：ladder 字段相对误差，只在"两条路径的
       ...
       字段值天然没有可比性。
     - **3b（决策依据，唯一的硬性决策门）：这批处理完之后立即做一次真实
       attention 读出，比较批量路径与严格串行参考各自最终 cache 状态给出
       的注意力输出的相对 L2 误差**。这一项**不需要任何簇对齐**——直接对
       同一批 query 分别跑一次 attention、比较输出张量，两条路径的簇怎么
       编号、有没有一一对应完全不影响这个比较是否良定义，因此是唯一在
       "聚类本身发生真实分歧"时依然能给出干净数字的一项，也是唯一直接
       回答"这个近似值不值得用"的一项——前两项（1、2）加上 3a 都是诊断
       信号（帮助归因"差在哪"），3b 才是决策依据。
