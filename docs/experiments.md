...
- **S0.8**：分歧率不是一个单一标量，必须拆成三项分别报告，理由是它们诊断的是
  ...
  1. **cluster assignment divergence（含 Ward 事件）**：用
     ...
     （`(slot,epoch)` 或 `resolve_final_slots` 给出的槽号都可以，标签本身
     怎么编号不影响结果），建一张 `K_A×K_B` 的列联表（contingency table）
  ...
  3. **最终 cache/readout 差异，拆成两个子项，硬性决策门只挂在 3b**——第 1
     ...
     - **3b（决策依据，唯一的硬性决策门）：这批处理完之后立即做一次真实
       ...
       > 因果性，3b 只用序列尾部的 query block**（此时两条路径的最终
       > cache 状态都已经构造完毕，序列尾部的 query 因果地有权看到整个
       > cache），不需要让 cache 构造和机制 B 的循环逐块交错对齐，两个
       > 阶段谁先谁后不重要，只要 cache 构造在 3b 要用到它的那个
       > query block 被处理**之前**完成。
       >
