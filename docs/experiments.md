...
- **S0.8**：分歧率不是一个单一标量，必须拆成三项分别报告，理由是它们诊断的是
  ...
  1. **cluster assignment divergence（含 Ward 事件）**：用
     ...
     **同时报告 Ward 事件本身的分歧**（批量路径触发的 Ward 合并次数、以及
     每次合并选中的候选对，与严格串行参考逐一比较）——Ward 决策错一次会
     级联影响后续所有共享该簇的 token，比单个 token 路由错位后果更重，值得
     单独一行，不要只并进上面的成对一致率里被平均掉。
  2. **segment/PAD overhead**：`segment_count_ratio = 批量路径总 segment 数 /
     ...
     而不是"有没有偏差"。
  3. **最终 cache/readout 差异**：批处理完这一批之后，比较批量路径与严格
     串行参考产出的 ladder（`k̄/v̄/w/p_lo/p_hi/sum_wp` 等全部字段）在**已经
     对齐到同一组 token 之后**（用第 1 项的 `(slot,epoch)` 身份做匹配，排除
     单纯因为槽号错位造成的伪差异）的相对误差，以及/或者在这批之后立即做
     一次 attention 读出、比较两条路径给出的注意力输出的相对 L2 误差。
     **这是唯一直接回答"这个近似值不值得用"的一项**——前两项是诊断信号
     （帮助归因"差在哪"），这一项才是决策依据。

  **决策门只挂在第 3 项上**：最终 cache/readout 的相对误差应 < 5%（沿用原来
  的数字，但现在明确它挂在哪一项）。第 1、2 项没有独立的通过/失败阈值，是
  诊断输出——**但必须报告**，因为若第 3 项超标，第 1/2 项决定了修法：如果是
  cluster assignment 分歧主导（尤其 Ward 事件分歧），要收紧 Phase 1 的批量
