...
- **S0.2（三重）**：①32k 下**unclipped** `K_eff` 应在 10² 量级；②曲线形状若是幂律，
  ...
  > 具体决策规则（含"取观测区间上界而不是只看渐近斜率"这条修正）见
  > `algorithm-spec.md` §5.6"从 `K_eff` 到 `c` 的决策规则"一节。
  >
  > **从 `K_eff` 曲线到生产 `c`（`K_max_default` 里的常数）的具体决策规则见
  > `algorithm-spec.md` §5.6"从 `K_eff` 到 `c` 的决策规则"**：包括拟合斜率怎么
  > 换算成 `c`、新增的"`K_max` 绑定率"这个 Stage 1/2 运行时指标该测多低才算够、
  > 以及加大 `c` 也压不下绑定率时应该转向重新评估曲线形状而不是继续调参。
