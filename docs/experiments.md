...
> `cache_serial`/`cache_batch` 两块分析基础设施（`cache_batch` 依赖的
> §5.4 批量近似路由目前完全没有实现），见下方"这句话的范围"一节。**S0.1
...
> 精确分解）；S0.3/S0.6/S0.7 虽然数据依赖上只需要机制 A，但隔离率/锚点去重/
> supersession 这几个分析逻辑本身还没写；S0.1 不需要 dump（纯 CPU 单测，见
> 下表），不在这个"数据依赖"讨论范围内。
...
> 严格串行路径这两侧的 cache 状态——`cache_serial` 由 §5.18 第 2 步"CPU 参考
> 实现的路由"（朴素串行版）构造，`cache_batch` 由 §5.4 Phase 1/2/3 的批量近似
> 路由构造，**`cache_batch` 这一侧目前在仓库里完全没有实现**（见
> `algorithm-spec.md` §5.18 第 0 步、第 2 步的更正框）。3b 在 `cache_serial`/
> `cache_batch` 都齐备之上，才**额外**需要
...
> CPU 参考路由本来就是"慢但正确"的测试脚手架、不是要部署的代码；§5.4 的批量
> 近似路由虽然确实没有实现，但它同样是 S0.8 这个分歧率决策门自己需要的比较
> 对象，不是"要不要投入 Stage 1"这道门在评估的生产代码；
