...
| `n_total` | 簇 `c` 的真实物理规模（token/entry 数，**不含 pad**），**单调不减，从不衰减，整数**。Ward 合并代价（§5.6）和 §5.8 的 O(log n) 空间界都用这个，不能用 `n_eff`——早期版本只有一个 `n_c` 两处混用，会让 Ward 把"历史长但被衰减过"的簇误判成小簇（§5.5/§5.6 更正框）|
| `n_unit_c` | 簇 `c` level 0 的累积**逻辑插入数**（真实 token + 历史 pad），单调不减，整数。**只喂 §5.11 的 `PAD_INSERT` 对齐计算**，不能用 `n_total`——早期版本用 `n_total` 算对齐会在第一次填充后系统性算错（§5.11 更正框）|
...
**新增的 buffer**（§5.13）：`centroid`、`n_eff`、`n_total`、`n_unit_c`、`p_hi_c`、`alive`、
