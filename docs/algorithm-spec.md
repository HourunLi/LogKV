...
0. **Stage 0 的 dump 脚本**（规格见 experiments.md）——它是所有 Stage 0 结论的输入，
   排在生产代码之前（§5.21-5）。S0.0–S0.7 和 S0.8 的第 1/2/3a 项的**数据依赖**都
   只需要这一步。
...
> **更正（这一轮修的）："第一段代码"这句话说的是 S0.0–S0.7 和 S0.8 的第
> 1/2/3a 项的数据依赖都只需要 dump，不是这些项只靠 dump 就能跑出结果；
> S0.8 的 3b 项额外还有它自己独立的、更晚的前置依赖，之前没有显式说清楚，
...
> 1/2/3a 成立的范围，跟对 S0.0–S0.7 成立的范围不是一回事：S0.0–S0.7 是
> "dump 完就能跑分析脚本"（S0.3/S0.6/S0.7 的分析器代码除外，见 §5.18 第 0
> 步的更正框），S0.8 1/2/3a 是"dump 只满足数据依赖，第 2 步和 §5.4 批量
> 近似路由这两块基础设施都还得先补上才能真正产出分歧率数字"。
>
