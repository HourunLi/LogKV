...
0. **Stage 0 的 dump 脚本**（规格见 experiments.md）——它是所有 Stage 0 结论的输入，
   ...
   > 路由 vs 严格串行参考"的分歧率，严格串行那一侧要等下面第 2 步的 CPU
   > 参考路由，批量近似那一侧（§5.4 Phase 1/2/3）目前在仓库里**完全没有
   > 实现**，缺的是比较的一侧，不是"第 0 步之外还差一点"。详见 CLAUDE.md
   > §0 与 `unused/semantic_stage0_dump.py` 模块 docstring 的显式 scope
   > 声明；§5.21-5 有完整分解。
1. **`log_kv_position.py` + 单测**（纯 CPU，不依赖任何 dump）。**S0.8 3b 的前置项**。
2. **CPU 参考实现的路由**（朴素串行版，慢但正确）——它是 S0.8 全部子项
   （1/2/3a/3b，不只 3b）的严格串行参考基准 `cache_serial`，不要跳过。

   > **更正（这一轮补的）：上一版说这一步（加 `CacheAttentionState` 迁移）是
   > "S0.8 3b 唯一需要在第 0 步之外补的前置代码"，这句话漏掉了 §5.4 的批量
   > 近似路由，需要改。** 这一步只给出 `cache_serial` 这一侧；S0.8 的**全部**
   > 子项（含 1/2/3a，不只 3b）都还需要另一侧 `cache_batch`，即 §5.4 Phase
   > 1/2/3 的批量近似路由给出的 cache 状态——**这一侧目前在仓库里完全没有
   > 实现**（见上方第 0 步的更正框、§5.21-5），不是"这一步做完 3b 就齐了"。
   > 3b 在 `cache_serial`/`cache_batch` 都齐备之上，才**额外**需要
   > `log_kv_slot_attention()`/`get_attention_state()` 按 §5.14/§5.20-B
   > 扩展出 `CacheAttentionState`/`slot_valid`/`M_s`——这是一次 breaking 的
   > API 迁移（不是"纯加法式改动"，见下方"更正"框与 §5.20-B 调用点迁移清单
   > 最后一行），需要在同一次改动里原子迁移全部约 30 处调用点（生产代码 6
   > 处 + 测试约 25 处），但改动本身是机械的位置解包换字段访问、不涉及新
   > 算法，见 §5.21-5 的更正框。S0.0–S0.7 不依赖这一步的 `cache_serial`，也
   > 不依赖 §5.4 的批量近似路由——它们只测单条路径自身的性质，不做 batch vs
   > serial 的比较。
...
> 状态——**这两者不是同一个来源**：`cache_serial` 由 §5.18 第 2 步"CPU
> 参考实现的路由"（朴素串行版）构造，`cache_batch` 由 §5.4 的批量近似路由
> （Phase 1/2/3）构造，**后者目前在仓库里完全没有实现**（见 CLAUDE.md
> §0、`unused/semantic_stage0_dump.py` 模块 docstring）——3b 还需要
...
> 参考路由）+ §5.4 的批量近似路由（未实现）+ `log_kv_slot_attention`/
> `get_attention_state()` 的接口扩展"，晚于第 0 步（dump 脚本本身），字面上
> 和"dump 脚本排在任何生产代码之前"冲突。
>
...
> 需要 `cache_serial`（第 2 步）和 `cache_batch`（§5.4 批量近似路由，未
> 实现）才能算出任何分歧率——区别只是 1/2/3a 不需要 3b 额外要的 `q_tail`/
> `CacheAttentionState` 接口扩展那一块。所以"dump 排在第一段代码"这句话对
> 1/2/3a 成立的范围，跟对 S0.0、S0.2–S0.7 成立的范围不是一回事：S0.0、
> S0.2–S0.7 是"dump 完就能跑分析脚本"（S0.3/S0.6/S0.7 的分析器代码除外，
> S0.1 完全不在此列——它不需要 dump，见 §5.18 第 0 步的更正框），S0.8
> 1/2/3a 是"dump 只满足数据依赖，第 2 步和 §5.4 批量近似路由这两块基础
> 设施都还得先补上才能真正产出分歧率数字"。
>
