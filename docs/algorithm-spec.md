...
一整段语义空洞问题随之消失。**这个修法的价值不止于此**：即便不做重定向，严格
按 `op_log` 记录顺序做**顺序重放**也已经能正确重建 forward 的 cache 结构（结构
操作和主操作在同一条时间线上，先合并后建簇的顺序天然消解了槽复用——这也是
下面"单测"一段要把"重放正确性"和"批量近似质量"分成两条不同断言的原因）；
重定向额外换来的是**`op_log` 本身对每个 token 的最终归属自解释**——不需要
"必须完整重放一遍才知道某个 token 最终去了哪个簇"，这对调试、S0.8 对拍、以及
任何直接读 `op_log` 字段的下游工具都更安全，是值得付出这点有界代价的性质，不是
可有可无的装饰。
...
——批量化路由本就需要一个地方暂存"这一批算出来的 op"，新增的只是"重定向改写
只发生在提交进持久 `op_log` 之前，只碰本地缓冲"这一条。
...
**重定向只改写"cluster 身份"字段，不是笼统地改"`op.cluster`"——不同 op 类型
的字段语义不同，必须按类型区分**：

| op 类型 | arg0 的语义 | 是不是重定向目标 |
|---|---|---|
| `NEW_CLUSTER` | 新簇所在槽（`slot_idx`，一旦建立即等价于"这个簇的身份"）| **是** |
| `NEW_SEGMENT` | 目标簇 | **是** |
| `JOIN` | 目标簇 | **是** |
| `PAD_INSERT` | 目标簇 | **是** |
| `CARRY`（调试）| 目标簇 | **是**（保持和上面四类一致，即使生产不写它）|
| `WARD_MERGE` | `keep_slot`（arg0）/ `free_slot`（arg1）——**这是物理槽指针，不是"簇身份"字段** | **否** |

`WARD_MERGE` 被排除是因为它的两个字段说的是"哪两个物理槽发生了合并"，这本身
就是历史记录，重放时按**它在日志里的位置**原样执行、不依赖任何"当前"槽映射；
如果重定向也去改写 `WARD_MERGE` 的 `keep_slot`/`free_slot`，等于篡改了"曾经
发生过的合并操作"这个既成事实，会破坏 §5.6 五步过程"重放只需要按顺序执行
结构操作"这条已经确立的前提。
...
见下方"同批多个 orphan 要不要互相 join"）时，若 K_max 已满、需要 Ward 合并腾位：
    ...
    2. ward_merge_only(keep_slot, free_slot)   # §5.6 的 1-3+5 步，立即执行，
                                                 # 物理内容此刻已经正确合并、
                                                 # free_slot 已 alive=false，
                                                 # 但还没有任何新簇写进去
    3. append WARD_MERGE(keep_slot, free_slot, -1) 到本地缓冲
    4. 重定向改写（向量化，一次 torch.where 覆盖整个本地缓冲，**只碰上面表格
       标"是"的那五类 op 的 arg0**，`WARD_MERGE`（含刚追加的这一条自己）不参与）：
           is_target_type = local_ops.type in
               {NEW_CLUSTER, NEW_SEGMENT, JOIN, PAD_INSERT, CARRY}
           local_ops.arg0 = where(is_target_type & (local_ops.arg0 == free_slot),
                                    keep_slot, local_ops.arg0)
       —— 只改本地缓冲里**已经写入**的条目（Phase 1 的全部 + Phase 2 到目前为止
          处理过的更早的 orphan，含刚追加的 `WARD_MERGE` 但它被类型排除），
          free_slot 这个数值此刻在这些条目里只可能来自"合并前的旧簇"，因为这
          次合并刚刚发生，之后任何新写入的 op 才会赋予 free_slot 新的含义
    5. slot_idx = free_slot   # 释放出来的槽立即交给这个 orphan（组）的新簇使用
       —— 这一步同时初始化簇级元数据：alive[slot_idx]=true，
          centroid/n_eff/n_total/p_hi_c 按这个 orphan（组）的内容设初值
          （不是"沿用旧簇残留值"，free_slot 此刻已经是一张白纸）
       append NEW_CLUSTER(slot_idx, 0, -1) 到本地缓冲，对应这个 orphan（组）
       里**到达顺序最早**的那个 token   # 写在第 4 步的重定向之后，天然不会
                                          # 被那次改写误伤——它引用的
                                          # free_slot 说的是"新簇"，不是"旧簇"
...
**为什么这个顺序（先合并、再记 `WARD_MERGE`、再重定向、最后才写新 op）是
正确性的关键，不是随意的先后安排**：第 4 步的重定向只碰**已经存在**于本地
缓冲里的条目，第 5 步的新 `NEW_CLUSTER` 是在重定向**执行完之后**才追加的，
因此不会被同一次 `torch.where` 误伤——这正是靠时间顺序消歧"槽 X 在合并前指
旧簇、合并后指新簇"这个歧义的方式，不依赖任何显式的版本号或额外字段。这比
屏蔽方案更简单，也更彻底：不需要 `touched_mask`，不需要判断"有没有候选"，
也就不需要顺延——Ward 的候选池永远是"全部 alive、非对角线的槽"，`K_max≥2`
时保证非空（§5.6），所以这条路径**不会失败**，每个 orphan 都在自己所在的
批次内拿到真实归宿。

**为什么这不违反"Phase 1 是一次性向量化 scatter"这个批量化前提**：第 4 步的
...
**Phase 3 的簇级元数据更新必须读"重定向之后、提交进持久 `op_log` 之前"的最终
本地缓冲，不能读 Phase 1 的原始输出**：重定向会改变本批内一部分 token 的最终
归属（任何原本 Phase 1 分配到某个后来被合并掉的槽的 token，最终归属会变成
吸收它的那个槽）。§5.5 的 centroid 在线更新、`n_eff`/`n_total`/`p_hi_c` 的
维护，如果仍然按 Phase 1 刚算出来时的（尚未被重定向修正的）分配去跑，会把这些
token 的 `k_raw` 计入错误的簇的 centroid 混合——这不是"影响一点点精度"的近似
误差，是把内容计入了一个**它最终并不属于**的簇，属于需要显式禁止的错误用法。
**硬规则：Phase 3 统一更新 centroid/`n_eff`/`n_total`/`p_hi_c` 时，唯一合法的
输入是第 4 步重定向执行完毕之后的本地缓冲**（等价于："先跑完整个 Phase 2（含
所有重定向），再跑 Phase 3"，本来就是三个 Phase 顺序执行的自然结果，这里只是
把"Phase 3 不能提前读取、也不能读取过期快照"这条隐含要求显式写出来，防止
实现时为了"提前流水线化"而错误地让 Phase 3 的某些统计在重定向完成前就开始算）。

**必须补的单测，拆成两条不同性质的断言，不能合并成一条**：

1. **重放正确性（S0.1，必须逐位精确，纯 CPU 可测）**：构造一个批内既有 Phase 1
   大量分配、又有 Phase 2 orphan 触发 Ward 合并、且合并候选恰好会撞上 Phase 1
   已分配槽位的合成场景，断言（a）重定向正确地把本批所有引用旧槽的条目改写成
   新槽，且只改了上面表格标"是"的字段，`WARD_MERGE` 的 `keep_slot`/`free_slot`
   逐位不变；（b）合并之后新建的 `NEW_CLUSTER` 没有被误伤，仍然指向刚释放的
   槽；（c）同一个新槽如果被组内多个 orphan 共享，恰好一条 `NEW_CLUSTER` +
   若干条 `JOIN`/`NEW_SEGMENT`，不多不少；（d）**整批处理完之后提交进持久
   `op_log` 的条目，按标准顺序重放算法执行，产出的 ladder/centroid 状态和
   "直接执行本批的批量向量化写入"这两条路径逐位一致**——这条测的是"`op_log`
   忠实记录了批量前向实际做了什么"，不是"批量前向的路由决策对不对"。
2. **批量近似质量（S0.8，允许有分歧率，不要求逐位一致）**：本场景（Ward
   合并撞上 Phase 1 已分配槽位）作为 S0.8 分歧率统计里**必须覆盖的一类特殊
   输入**，衡量"批量路由（含这里的重定向机制）的最终路由决策"和"严格串行、
   每处理一个 token 就重算一次 centroid/`p_hi_c`/`alive` 的基准路由"之间差多
   少。**这条不要求、也不应该要求逐位一致**——Phase 1 冻结 centroid 和
   `p_hi_c` 本来就是一个已经承认、已经在测的近似（§5.4 开头"这是一个近似"那
   段），Ward 合并只是这个近似在"K 已满、批内密集触发新簇"这种极端情形下的一个
   特例，不应该单独拔高到"必须和严格串行版逐位相同"这个不属于它的正确性等级
   ——那样会把"重放准不准"和"批量决策的近似质量"这两个不同维度的问题混成
   一个，上一版的单测描述正是这么混的，这里拆开。
...
**上面 1-3+5 步是 `ward_merge_only(keep_slot, free_slot)` 这一个 primitive 的
完整内容——它只做合并和释放，不写新簇。** 早期版本把"释放槽位"和"新簇写进去"
并成同一步（原第 4 步"释放其中一个槽位，新簇写进去"），这在单 orphan、非批量
的场景里读起来无害，但 §5.4 的批量路由需要把"合并"和"在释放出的槽里建什么"
拆成两个独立步骤——`ward_merge_only` 只负责前者，**"新簇写进去"是调用方的职责，
不属于这个 primitive**，调用方在这里就是"触发合并的那个 novelty token"，它接下来
会走 §5.3 的 `NEW_CLUSTER` 路径，把这个刚释放的槽当成自己的新簇写进去。这个
拆分是 §5.4 批量路由小节里 Ward 合并与重定向机制的前提，不是本节自己需要，但
放在这里定义，供 §5.4 引用。
...
**硬性不变量（`ward_merge_only` 必须满足，不是"大概率如此"）**：第 3 步
...
for op in op_log:
    if op.type in {NEW_CLUSTER, JOIN, NEW_SEGMENT}:
        ...
        ward_merge_only(op.keep_slot, op.free_slot)   # §5.6 的 1-3+5 步，只合并
