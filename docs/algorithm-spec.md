...
| `B′` | `log_kv_cluster_entries` | `128`（当前 Stage-0 脚本默认） | ladder 每级 entry 数；同时是**无损阈值**（§5.9）|
...
**必须有 CPU 参考实现和对拍单测（实现单测，纯 CPU 可测——不是 Stage 0 的
...
> 补的单测"（重放正确性、Phase 3a/3b 元数据更新正确性、Ward 合并的
> `level_count` 不变量、padding 槽的完整字段对拍），测的都是 §5.3/§5.4
> 批量路由、Ward 合并、段对齐填充、`op_log` 重放这些**实现本身**的正确性
> ——和 `experiments.md` 表格里精确定义的 S0.1（`log_kv_position.py`/§5.14
> 的纯 CPU 单测）完全是两回事，只是碰巧都"纯 CPU、不需要 GPU dump"，此前
> 图省事借用了同一个标签。这个巧合掩盖了一个真实风险：CLAUDE.md §0 反复
> 强调"S0.1 唯一的前置依赖是 `log_kv_position.py` 还没写"，读者如果把这条
> 结论套用到这些其实依赖 §5.4/§5.6/§5.11/§5.21-2 实现进度的测试上，会
> 误以为它们也只等 `log_kv_position.py`——它们真正等的是各自测的那块路由/
> 重放机制本身先写出来。**这些测试今后统一称"实现单测"，不借用任何 S0.x
> 编号**——它们验证的是 Stage 1 相关模块自身的实现是否正确，不属于 Stage 0
> 的 S0.x 决策门实验框架，两套编号不共享。
...
它的返回值和调用方式已经被 `resolve_final_slots`、重放正确性
实现单测等多处依赖，它足够简单可信的原因正是"只维护解析最终归属需要的最小
...
def scan_op_log_for_ward_events(
    ...
    """只读、离线，S0.8 专用，不是 forward/backward/重放正确性实现单测依赖的东西——和
...
1. **重放正确性（实现单测，不是 S0.1——见 §5.4"实现单测"更正框，必须
   逐位精确，纯 CPU 可测，只覆盖 attention 需要的
   张量，不覆盖 centroid 类元数据——理由见下）**：构造一个批内既有 Phase 1
   大量分配、又有 Phase 2 orphan 触发 Ward 合并、且合并候选恰好选中**本批内
   刚建立的簇**（就是上面反例描述的场景，这条实现单测必须专门覆盖这一类输入，不能
...
3. **Phase 3a/3b 元数据更新的正确性（实现单测，不是 S0.1，整数字段逐位精确、浮点字段
...
> **2026-08-25 更新：`c=1` 已被 Stage-0 clipped gate 证明只是占位，不是够用默认。**
> 在 32k 数据上，`K_max=128` 时 `lambda_rel=1.0` 的 `K_max_binding_rate` 仍约
> 80.8–80.9%，`lambda_rel=0.875` 约 89.1%。若只按“绑定率 <10%”校准，
> `lambda_rel=1.0` 需要 `K_max≈280`，即 `c≈19`；若要覆盖 unclipped 平均
> `K_eff≈310`，则 `c≈21`，加 10% 均值余量约 `c≈23`。这些数只是
> **binding-only/no-Ward 上界**，不是默认推荐：同一批数据里 `K_max=128` 的
> `lambda_rel=1.0` exact-entry 已约 72.7%，接近 unclipped 的约 74.3%；`K_max=64`
> 也有约 67.5%。因此当前生产粗估是 `K_max≈64–128`，即 32k 下 `c≈4.3–8.5`，
> 优先试 `c≈6–8`。`K_eff` 覆盖只回答“怎样几乎不触发 Ward”，不回答“needle 是否
> 已被不可接受地压缩”。
...
2. **新增一个必须在 Stage 1/2 测的运行时指标：`K_max` 绑定率**——一次 eval/训练
   ...
   决策门里讲清楚），绑定率高也不直接等于 needle 质量下降。它只说明 Ward 预算转移
   很活跃，必须和 S0.3 的 needle 最终 entry 质量、Ward 是否触碰 needle、以及 S0.6
   成本一起读。
   - **绑定率低**：路由接近 unclipped，是保守好消息。
   - **绑定率高但 needle final-entry 质量接近平台**：不为 binding 单独加 `c`。
   - **绑定率高且 needle final-entry 质量明显低于平台**，先加倍 `c` 重测——如果加大
     `c` 后质量明显恢复，说明只是常数没给够；如果只降 binding、不涨质量，就不要继续
     用内存买一个对目标无效的指标。
   ...
   **当前 Stage-0 clipped gate 的实际代入结果（32k，已有数据，不是新实验）**：
   `lambda_rel=1.0`、`g_max={inf,8192,4096}` 的 unclipped 平均新簇尝试数约
   309–311；按 `<10%` 绑定率反推 `K_max≈279–281`，所以 `c≈18.6–18.7`，
   这是 binding-only/no-Ward 上界。实际 S0.3 质量并不要求覆盖这个上界：
   `K_max=128` exact-entry 约 72.7%，接近 unclipped 的约 74.3%；`K_max=64`
   约 67.5%。因此当前生产粗估先取 `K_max≈64–128`，即 `c≈4.3–8.5`，优先
   试 `c≈6–8`。
...
2. 合并两条 ladder —— 逐层调用 §5.12 的共享折叠原语 carry_into_level/
   saturating_top_carry（**这一轮改的**：不再是本节自己的"保留最新 B′，其余
...
**"总数 ≤ B′ 时不触发任何合并"是整个机制的关键**：合并两个簇**不必然合并它们的
entry**。**这一轮更正**：这个条件不再是本节 pseudocode 里一个显式可见的
`if` 分支——它现在是 §5.12 `carry_into_level`/`saturating_top_carry` 内部
`while len(combined) > B′` 循环体的隐式否定（循环一次都不触发，`combined`
原样写回），语义完全不变，只是挪进了共享原语内部，不需要 Ward 合并这一侧
重新实现一遍判断。

- 两个**小簇**合并 → 归并后该层占用仍 `≤ B′` → **entry 原样共存，无池化损失**（**注意这是
  暂态而非不变量，反复合并会累积占用直至越过 `B′`，见上面的警告框**），只是 centroid
  变成混合体（影响后续路由，不影响已存内容）。
- 两个**大簇**合并 → 每层都是 2B′ 个 → 全线触发折叠 → **真实的分辨率损失**。
...
**硬性不变量（合并结束时必须满足，不是"大概率如此"）**：第 2 步
结束时，对**每一层** `ℓ`，`level_count[keep_slot, ℓ]` 必须精确等于该层归并
之后新 ladder 里真实存活的 entry 数。**这一轮更正**：这条不变量现在由
`carry_into_level`/`saturating_top_carry` 自身保证（两者内部都以
`level_count[cluster, level] = len(combined)` 结束，是函数返回前的最后一步，
不需要调用方另外记得写）——不再是"§5.6 这一侧的合并循环必须自己记得同步更新，
...
**对应的单测（实现单测，不是 S0.1，必须有，不是"w=0 恒等元"那条的可选补充）**：`merge(A, pad)`
...
> 新增负担。关键认识：**只要 `B′` 是 `2^ℓ_block` 的倍数**（当前 Stage-0 默认 `B′=128`，
> `ℓ_block∈{0,1,2}` ⇒ `2^ℓ_block∈{1,2,4}`，均整除 128），"level 0 当前占用数
...
> 成立，回到"第一次填充后就分叉"的老问题。当前默认值 `B′=128` 对 `ℓ_block∈{0,1,2}`
...
> **更正（这一轮修的，P1，第一次）：上一版说"不需要新机制，`compact` 本来就不要求
> 两侧等宽"，这句话本身没错，但回答的不是真正的问题。** `compact()` 这个纯函数确实不
...
>
> **更正（这一轮修的，P1，第二次，推翻第一次修法里的核心不变量）：上一次修复
> 声称"抵达顶层的进位块宽度恒为 `B′`"，这条不变量是假的。** 反例直接来自 §5.6
> 的 Ward 合并：`ward_merge_only` 步骤 2 逐层归并两个簇的 ladder 时，某一层的
> "其余"（超出 `B′` 的部分）宽度是 `总数(≤2B′) − B′`，这个数**可以是 1 到
> `B′` 之间的任意值，不保证是 `B′` 也不保证是偶数**——例如 `B′=8`，两个源簇在
> 某层分别有 8 个和 5 个 entry，归并后总数 13，"其余" 5 个必须以某种方式压缩
> 后继续进位，宽度既不是 `B′`（8）也不是偶数，"两个 `B′` 宽的输入拼成 `2B′`
> 按 `(2i,2i+1)` 配对"这条 `compact()` 的既有用法在这里根本用不上——上一版的
> "先证明一条支撑简化的不变量"那段证明，只证了**普通层间进位**（层内容要么空、
> 要么恰好 `B′`）这一条路径，完全没检查 Ward 合并注入的进位是否也满足同一个
> 前提，而它明确不满足。`saturating_top_carry` 原来的实现假设"收到的永远是
> `B′` 宽的块，直接和既有内容做一次 `compact`"，遇到 Ward 合并级联上来的非
> `B′` 宽块会在 shape 上直接不兼容。
>
> **进一步深挖发现，问题比"Ward 合并的进位宽度不固定"更深一层：连普通单 token
> 逐个 append 的进位机制本身，在这份文档里都从未被精确定义过。** §5.7 只有一句
> "level ℓ 满（B′ 个）时，成对合并进位到 ℓ+1"，`append_to_ladder`（§5.21-2 重放
> 一节引用的那个函数）被反复引用但从未给出内部实现。逐步验证多个候选语义后
> （"整层 B′ 个一次性两两合并"违反"新 token 单独精确保留"的 §5.9 承重论证；
> "新到的单 token 和既有内容做 `compact`"要求既有内容也是宽度 1，与"level 恰好
> `B′` 个 entry"矛盾），确认唯一自洽的语义是：**level 未满（`< B′`）时新 entry
> 直接追加，不触发任何合并；level 恰好满（`= B′`）时收到第 `B′+1` 个 entry，
> 从当前内容里取**时间序最早的两个**做一次 `merge()`（§5.10），产出 1 个
> 双倍权重的 entry 向上一层进位，本层保留"未被动过的 `B′-2` 个 + 新到的 1 个"，
> 变成 `B′-1` 个（下一次 append 补满回到 `B′`）。** 这个语义此前完全没有写出来
> 过，是这一轮才第一次显式定义，不是"之前写错了"，是"之前没写"。**关键结论：
> 在这个语义下，任何一次向上进位的块宽度恒为 1**（每次 level 满员溢出只产出
> 1 个合并 entry），这和 Ward 合并注入的、可以宽达 `B′` 的块是两种不同来源、
> 不同宽度分布的输入——`saturating_top_carry` 必须同时正确处理两者，不能假设
> 任何一个固定值。
>
> **修法：不再假设固定宽度，改成一个显式的、有界折叠循环，两类进位来源共用同一个
> 原语。** 核心操作永远是"取当前最老的两个 entry 做一次 `merge()`"，重复到不再
> 超出 `B′`——这是唯一不依赖"输入宽度恰好是某个值"这个假设、且能保持
> §5.7"时间序相邻配对"这条既有事实的做法。非顶层（普通层）与顶层的区别只在于
> "折叠出来的东西送去哪"：非顶层把折叠结果**送到下一层**（不留在本层，本层因此
> 可能降到 `B′` 以下，下次 append 再补满）；顶层没有下一层，折叠结果**留在原地**
> （与本层剩下的内容拼在一起，恰好回到 `B′`，一个都不丢——这正是"饱和累加"字面
> 的意思：不丢弃，只折叠）。
>
> ```python
...
>     驱动逐层级联，两处调用方的驱动逻辑不同，但都复用这一个原语。"""
>     existing = read_level(cluster, level)              # 0..B′个，时间序、最老在前
>     combined = list(existing) + list(incoming_block)     # 仍是时间序
>     ejected = []
>     while len(combined) > B′:
>         merged = merge(combined[0], combined[1])           # 当前最老的两个，merge() 见 §5.10
>         combined = combined[2:]
>         ejected.append(merged)                              # 不放回 combined，避免下一轮
>                                                                 # 又把它连着下一对折进去，变成
>                                                                 # 链式合并而不是两两配对
>     write_level(cluster, level, combined)                  # <= B′
>     level_count[cluster, level] = len(combined)
>     return ejected                                          # 空列表代表这一层已经吸收完，
>                                                                 # 不需要继续往上
>
>
...
>
> ```python
> def append_to_ladder(k, v, pos, cluster, segment):
>     incoming = [make_level0_entry(k, v, pos, segment)]      # 宽度恒为 1
>     for level in range(0, top):
>         ejected = carry_into_level(cluster, level, incoming)
>         if not ejected:
>             return                                            # 没有溢出，到此为止
>         incoming = ejected                                    # 宽度恒为 1（见上面的推导），
>                                                                   # 但驱动循环本身不依赖这个事实
>     saturating_top_carry(cluster, incoming)                  # 级联到了顶层
> ```
>
> **`merge()` 复用的字段与既有约定完全一致**——`k̄_raw/v̄/w/p_lo/p_hi/sum_wp`
> 走 §5.10 已给出的加权合并公式，rank-1 `σu/σ2/γa/γb/γ`（若启用）走既有的
> Chan-merge 分支，pad 槽（`w=0`）走既有的"全字段显式清零、`merge()` 与 `w=0`
> 搭档时是恒等元"规则（§5.19-3）——**没有任何字段公式改变**，改变的只是"什么
> 时候调用、调用几次、结果流向哪里"这层控制流。**不再需要 `w=0` 补位凑偶数**
> ——上面两个循环都是"每轮固定取当前最老的两个"，不要求输入总数是偶数，奇数
> 输入时循环会在某一轮之后自然停在 `≤B′`，不存在"剩一个凑不成对"的情形。
>
> **`level_count[cluster, top]` 不再是"只有 `{0,B′}` 两个取值"**——这是上一版
> 单测要求里一个连带错误，一并更正：顶层第一次被触碰时，是像任何其它层一样从
> 0 逐个 token **递增**填到 `B′`（`saturating_top_carry` 在 `len(combined)≤B′`
> 时循环一次都不触发，直接原样写入），中间会经过 `1,2,...,B′-1` 每一个值，
> 不是从 `0` 直接跳到 `B′`；只有当它**第一次**到达 `B′`、后续再收到任何进位时，
> 才会触发折叠并**从此钉死在 `B′`**（`combined` 折叠后恒等于 `B′`，不再降低）。
> 正确的不变量是"单调不减，上界 `B′`，一旦达到 `B′` 便永久保持"，不是"只在
> `{0,B′}` 间跳变"。
>
> **replay 不需要新的 `op_log` 条目类型。** 和普通二进制进位一样（`CARRY` 是
> debug-only、非权威，见 §5.21-2），这套折叠完全是给定"哪些 entry、按什么顺序
> 进了哪一层"就能确定性重算的纯函数，不依赖任何额外记录——replay 只需要让自己
> 模拟的进位状态机调用同一份 `carry_into_level`/`saturating_top_carry`。**要求**：
> forward 路由和 op_log replay 必须调用同一份实现（不允许各自平行重写一份），
> 与 `ward_merge_only`（§5.6）、`PAD_INSERT` 扫描（§5.11）等已经确立的
> "forward/replay 共享同一 primitive"纪律一致。
>
> **必须补的单测**（覆盖此前不存在、这一轮才发现需要测的三类场景，不是简单
> 复用上一版的单一场景）：
> 1. **顶层渐进填充**：单簇注入恰好 `B′` 个 token，断言顶层 `level_count`
>    依次经过 `1,2,...,B′`（不是直接跳到 `B′`），且这 `B′` 个 entry 逐位等于
...
**这次修正没有引入任何新的合并数学**——`merge()`（§5.10）的字段公式一行未改，
唯一的新东西是"用一个有界循环反复调用它、直到不再超出 `B′`"这层控制流，
以及由此推导出的、此前从未写出过的普通逐 token 进位语义（§5.7 只给了一句
概括，从未给出可执行定义）。要改的是 `_counts[ell]` 那套"同层等宽"的簿记
假设（**和对齐填充要改的是同一处，见 §5.19-2**），以及本节新增的这套折叠
控制流本身。
...
> **`append_exact_tokens()` 的新契约（这一轮补的，前面 §5.20-B 调用点迁移
> 清单已经说过"输入输出都要跟着换成 `CacheAttentionState`"，但从未给出具体
> 签名，导致 `experiments.md` S0.8 3b 的调用点还在按老式 3-元组
> `k_all, v_all, w_all = append_exact_tokens(slot_k, slot_v, slot_w, k_new,
> v_new)` 写，和这里的决定对不上）**：
>
> ```python
> def append_exact_tokens(state: CacheAttentionState, k_new, v_new) -> CacheAttentionState:
>     """把 in-flight chunk（k_new/v_new，post-RoPE，隐式 w=1）拼在 state 的
>     pooled+recent 之后（§5.14"现有 causal_tail 机制"一节的既有 flatten 顺序，
>     pooled 在前、exact 在后，原样保留）。返回一个新的 CacheAttentionState：
>     slot_k/slot_v/slot_w 是拼接后的结果；slot_valid/M_s 及全部 rank-1 字段
>     原样从 state 透传、不随拼接改变宽度——它们只覆盖 pooled 前缀
>     （S_pooled 宽度不变），log_kv_slot_attention 已经把"S_pooled 之外的位置
>     隐式 slot_valid=True/M_s=1"当作既有约定处理（上面"S_pooled 之外的字段"
>     一节），调用方不需要、也不应该把 slot_valid/M_s 拼宽去覆盖新拼接的尾部。"""
> ```
>
> 输入从"三个裸张量 `slot_k,slot_v,slot_w`"改成"一个 `CacheAttentionState`"，
> 是因为调用方（S0.8 3b、`model.py` 的三处流式调用点）手上拿到的本来就是
> `get_attention_state()` 返回的 `CacheAttentionState`，中途拆成三个位置参数
> 传进去、再把返回的三元组重新包一层，纯粹是多余的拆包-重组，且正是这份
> 规格反复在抓的"位置参数摆错顺序"这一类风险的又一个来源。**`k_new`/`v_new`
> 仍是裸张量**（不强求调用方也把 in-flight chunk 包成 `CacheAttentionState`
> ——它没有 `slot_valid`/`M_s`/rank-1 字段这些概念，硬套这个类型只会制造一堆
> 无意义的 `None`）。
...
Σ/Γ 的旋转数学是两件正交的事：Σ/Γ 的**锚点旋转数学**正确性已经由本节这两条
单测独立覆盖（嵌套精确性、坐标系回归，纯 CPU、不需要批量/串行两条路径对拍）
——**这一轮更正**：这不等于"Σ/Γ 的正确性"这个更笼统的说法，尤其不覆盖"批量
近似路由/compaction 下 Σ/Γ 的聚合状态本身是否也和严格串行参考一致"，后者
完全没有测试覆盖，留到下面"这个范围决定留下一个必须显式接住的后果"一节和
未来可能新起的 3c（不要只读到这一句就以为 Σ/Γ 已经被 3b 或本节单测整体
验证过）。若 3b 也
...
0. **Stage 0 的 dump 脚本**（规格见 experiments.md）——它是所有 Stage 0 结论的输入，
   排在生产代码之前（§5.21-5）。S0.0、S0.2–S0.7 和 S0.8 的第 1/2/3a 项的**数据
   依赖**都只需要这一步；**S0.1 不在此列**——它是 `log_kv_position.py`（还没写）
   的纯 CPU 单测，本来就不需要 dump 提供任何数据，混进"S0.0–S0.7 都靠 dump"这个
   总括会让读者误以为 S0.1 也在等 dump，实际上它连 dump 都用不上，卡住它的是
   `log_kv_position.py` 本身还没写（见下面第 1 步）。

   > **更正（这一轮补的）："只需要这一步"说的是数据依赖，不是"这一步做完就能
   > 跑出结果"，两者不能划等号。** S0.3（needle 隔离率）/S0.6（锚点去重
   > `E[M]`）/S0.7（supersession）的分析器代码还没写，不是"跑一下就有数"；
   > **S0.8 无论第几项都还跑不了，1/2/3a 和 3b 一样**——它测的是"批量近似
   > 路由 vs 严格串行参考"的分歧率，两侧都要等下面第 2 步：`cache_serial`
   > 和 `cache_batch` 这两份朴素 CPU 参考实现目前在仓库里**都完全没有
   > 实现**，缺的是比较的两侧，不是"第 0 步之外还差一点"。**`cache_batch`
   > 是 §5.4 Phase 1/2/3 算法的朴素 CPU 实现，不是第 4 步"多簇路由 + 向量化"
   > 那份生产实现——两者是不同的制品，混淆会把 S0.8 变成要等 Stage 1 投入
   > 决策完成才能跑，见下面第 2 步的更正框。** 详见 CLAUDE.md §0 与
   > `unused/semantic_stage0_dump.py` 模块 docstring 的显式 scope 声明；
   > §5.21-5 有完整分解。
1. **`log_kv_position.py` + 单测**（纯 CPU，不依赖任何 dump）。**S0.8 3b 的前置项**。
2. **CPU 参考实现（朴素、慢但正确，两份）**——`cache_serial`（§5.3 严格串行
   算法的朴素实现，逐 token 用当时最新 centroid 处理）和 `cache_batch`（§5.4
   Phase 1/2/3 批量近似算法的朴素实现，把 Phase 1/2/3a/3b 的伪代码逐字翻译成
   普通 Python 循环，**不做任何向量化**——不是下面第 4 步要写的生产实现，见
   下方"更正"框）。两者是 S0.8 全部子项（1/2/3a/3b，不只 3b）互相比较用的
   基准状态，不要跳过。

   > **更正（这一轮补的）：S0.8 需要的 `cache_batch` 和第 4 步"多簇路由 +
   > 向量化"不是同一件东西，混为一谈会造成循环依赖，必须拆开。** 上一版把
   > "§5.4 的批量近似路由"笼统地描述成"Phase 1/2/3 向量化"，而下面第 4 步
   > 恰好又叫"多簇路由 + 向量化（§5.4 三阶段）"——字面读起来 S0.8 需要的
   > `cache_batch` 就是第 4 步的产物；但第 4 步正是 CLAUDE.md §0"视 Stage 0
   > 结果决定是否继续 Stage 1"要等 Stage 0 结果才投入的那部分生产实现。这样
   > S0.8（Stage 0 的决策门）就要先有第 4 步（被这道决策门挡住的 Stage 1
   > 投入）才能跑，逻辑倒转。
   >
   > **修法：把"§5.4 描述的算法"和"§5.4 算法的生产级向量化实现"拆成两个
   > 不同的制品，跟 §5.3 严格串行算法早就有的两层结构完全对称**——严格串行
   > 算法只有这一步的朴素参考实现（`cache_serial`），从不追求高吞吐，没有
   > 生产版；批量近似算法则同时有朴素参考（`cache_batch`，**这一步一并写**，
   > 目前完全没有）和生产向量化实现（第 4 步，仍然按 Stage 0 结果决定要不要
   > 投入）。`cache_batch` 的朴素实现代价和 `cache_serial` 是同一个量级——
   > 本来就是 S0.8 用的测试脚手架，不是要部署的代码，**S0.8 比较的是
   > 这两份朴素 CPU 实现**（批量近似算法 vs 严格串行算法，回答"批内冻结
   > centroid/`p_hi_c` 这个近似本身丢多少精度"），从不涉及第 4 步。第 4 步
   > 做完之后，"生产向量化实现 vs 这一步的 `cache_batch` 朴素参考"是一次
   > 独立的、Stage 1 内部的实现正确性对拍（回答"向量化写对了没有"，第 4 步
   > 自己的说明有交叉引用），跟 S0.8 是两件事——S0.8 不受它是否已经动工
   > 影响，它也不受 S0.8 是否已经跑完影响。
   >
   > `log_kv_slot_attention()`/`get_attention_state()` 按 §5.14/§5.20-B
   > 扩展出 `CacheAttentionState`/`slot_valid`/`M_s` 是 3b 在 `cache_serial`/
   > `cache_batch` 都齐备之上才**额外**需要的——这是一次 breaking 的 API
   > 迁移（不是"纯加法式改动"，见下方"更正"框与 §5.20-B 调用点迁移清单最后
   > 一行），需要在同一次改动里原子迁移全部约 30 处调用点（生产代码 6 处 +
   > 测试约 25 处），但改动本身是机械的位置解包换字段访问、不涉及新算法，
   > 见 §5.21-5 的更正框。S0.0–S0.7 不依赖这一步的任何一份 CPU 参考实现——
   > 它们只测单条路径自身的性质，不做 batch vs serial 的比较。
...
4. **多簇路由 + 向量化**（§5.4 三阶段的生产实现，本节最终视 Stage 0 结果
   决定是否投入的核心工程量之一）——完工后拿第 2 步新增的 `cache_batch`
   朴素参考实现测一次分歧率，但这是 Stage 1 内部的**实现正确性**对拍
   （向量化版本有没有忠实复现同一个算法），不是 S0.8（S0.8 在第 2 步就已
   经用两份朴素 CPU 参考实现测完了，见第 2 步的更正框），不要混为一谈。
...
- **对实现的具体要求**：批量写入必须**保序**——一个 flush 批内，凡是路由到同一个
  ...
  会重排到达顺序或跳过中间进位状态的捷径。
- **必须落地成单测**（实现单测，不是 S0.1/S0.8，和上面"重放正确性"是同一类断言、
  只是聚焦 carry 这个子机制，纯 CPU 可测）：构造一批同簇 token，分别用
...
> **更正（这一轮修的）："第一段代码"这句话说的是 S0.0、S0.2–S0.7 和 S0.8 的第
> 1/2/3a 项的数据依赖都只需要 dump（S0.1 不在此列——它是
> `log_kv_position.py`（还没写）的纯 CPU 单测，不需要 dump 提供任何数据，见
> §5.18 第 0 步的更正框），不是这些项只靠 dump 就能跑出结果；S0.8 的 3b 项
> 额外还有它自己独立的、更晚的前置依赖，之前没有显式说清楚，
> 字面读会和 §5.18 的步骤顺序打架。** S0.8 3b（attention 读出的相对 L2
> 误差，§6 的硬性决策门）需要 `cache_batch`/`cache_serial` 这两个 cache
> 状态——**这两者都在 §5.18 第 2 步构造，但对应两个不同的算法**：
> `cache_serial` 是 §5.3 严格串行算法的朴素 CPU 实现，`cache_batch` 是
> §5.4 Phase 1/2/3 批量近似算法的朴素 CPU 实现（把 Phase 1/2/3a/3b 的伪
> 代码逐字翻译成普通 Python 循环，不做向量化）——**`cache_batch` 不是 §5.18
> 第 4 步"多簇路由 + 向量化"那份生产实现，两者是不同的制品，混淆会让 S0.8
> 变成要等 Stage 1 投入决策完成才能跑，见 §5.18 第 2 步的更正框**；这两份
> 朴素实现目前都完全没有写（见 CLAUDE.md §0、`unused/semantic_stage0_
> dump.py` 模块 docstring）——3b 还需要 `log_kv_slot_attention()`/
> `get_attention_state()` 已经按 §5.14/§5.20-B 扩展出
> `CacheAttentionState`/`slot_valid`/`M_s`（3b 的设计明确要求"直接、
> 原样传递生产函数的返回值，不做手工重建或平行实现"，理由是避免测试代码和
> 生产代码分叉——这条原则本身没有问题，但它意味着这两块代码必须先存在）。
> 按 §5.18 的步骤编号，这是"第 1 步（`log_kv_position.py`）+ 第 2 步
> （`cache_serial`/`cache_batch` 两份朴素 CPU 参考实现）+
> `log_kv_slot_attention`/`get_attention_state()` 的接口扩展"，晚于第 0 步
> （dump 脚本本身），字面上和"dump 脚本排在任何生产代码之前"冲突。
>
> **同一个 `cache_serial`/`cache_batch` 缺口不是 3b 独有的**，S0.8 的第
> 1/2/3a 项（cluster assignment/Ward 事件、segment+PAD 开销、非 attention
> 部分的最终 cache/readout 误差）同样要比较批量路径与严格串行路径，同样
> 需要 §5.18 第 2 步这两份朴素 CPU 参考实现才能算出任何分歧率——区别只是
> 1/2/3a 不需要 3b 额外要的 `q_tail`/`CacheAttentionState` 接口扩展那一块。
> 所以"dump 排在第一段代码"这句话对 1/2/3a 成立的范围，跟对 S0.0、
> S0.2–S0.7 成立的范围不是一回事：S0.0、S0.2–S0.7 是"dump 完就能跑分析
> 脚本"（S0.3/S0.6/S0.7 的分析器代码除外，S0.1 完全不在此列——它不需要
> dump，见 §5.18 第 0 步的更正框），S0.8 1/2/3a 是"dump 只满足数据依赖，
> 第 2 步的两份朴素 CPU 参考实现都还得先补上才能真正产出分歧率数字"。
>
> **这不是需要靠改期望解决的矛盾，是"生产代码"这个词在两处指代的范围不同，
> 需要把边界画清楚。** CLAUDE.md §0 用 Stage 0 的结果决定"是否继续 Stage 1
> （生产代码）"，真正被这道门挡住、需要先看到信号才值得投入的，是 §5.18
> 第 3–6 步——多簇路由的**生产向量化**实现、段对齐填充、`op_log` 训练路径
> 重放，这些是本方案唯一的、有实际工程量和回退风险的核心投入。3b 依赖的
> 两块东西性质不同：
> 1. **§5.18 第 2 步的两份 CPU 参考实现**（`cache_serial` 和 `cache_batch`）
>    在自己的定义里就是"朴素、慢但正确"——`cache_batch` 是 §5.4 Phase
>    1/2/3 算法的朴素 Python 翻译，不做向量化，**不是第 4 步要写的生产
>    向量化实现，两者是不同的制品**（见 §5.18 第 2 步的更正框，那里有完整
>    论证）；这两份朴素实现存在的唯一目的是当分歧率测试的对照基准（S0.8
>    本来就需要它们，这条依赖从第一版起就写在第 2 步的说明里，不是这一轮
>    新加的），不是要部署的代码，写它们不构成"要不要投入 Stage 1"这个
>    决策的组成部分。
