...
**下一步方向（2026-08-10 整理成分阶段路线图，见 6.5 补充的"稠密能力是否被侵蚀"假设）**：
...
"+pin" 从"比 vanilla 还差"变成至少不拖累、理想情况下有提升。

*阶段 3 —— 独立的一条线，搞清楚 salience 质量的天花板在哪（见 6.5）：*
(f) 给 `eval.py` 的 `LogKVLM` 加一个"dense 模式"开关（或写独立小脚本），对比
base checkpoint 和 CPT 后 checkpoint 在**稠密**注意力下的 NIAH 表现，判断当前
CPT 训练是否在侵蚀 pin 打分依赖的稠密检索能力；
(g) 视 (f) 结果决定：如果确认被侵蚀，考虑 CPT 训练配比里混入稠密注意力批次；如果
本来就弱、不是 CPT 的锅，再评估要不要做专门的检索能力提升（辅助 loss / 检索类数据
继续预训练，投入较大，优先级低）。

*阶段 4 —— 只有前面都做完、下游指标依然不理想时才考虑，成本最高：*
(h) 6.1 节"训练从没见过 pin 态"的训推一致训练——需要重新设计训练前向 + 重新跑
CPT，只有在选择质量和稠密能力都确认没问题、但 pin 仍不起作用时才值得投入。
