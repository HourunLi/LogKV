...
cfg = load(sys.argv[1])
banned = sorted(k for k in cfg if k.startswith("eval_log_kv_"))
if banned:
    raise SystemExit(
        "eval_log_kv_* keys are no longer supported; train and eval share the same LogKV config. "
        "Remove: " + ", ".join(banned)
    )
...
def semantic_budget(k_max, b_prime, n):
    ...
    anchors = 1 if cfg.get("log_kv_semantic_anchor_mode") == "mid" else 3
    return l_alloc, persistent, persistent * anchors
